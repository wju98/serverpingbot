...
servers must respond a ping. If any login server fails to do so, the login servers are considered to be offline.
...
async def on_message(message):
    ...
    if message.content.startswith('!ping') and message.author.id == 258064566456549387:
        ...
        this bot, aka me. For testing/demonstration purposes only.
        """
    ...
    for rm in react_messages:
        ...
        for reaction in react_message.reactions:
            async for user in reaction.users():
                if user.id != configpingbot.client_id:
                    users.add(user)
...
client.run(configpingbot.TOKEN)
